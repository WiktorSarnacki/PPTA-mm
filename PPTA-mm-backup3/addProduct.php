...
$sql = "INSERT INTO produkty (prod_name, price, description, user_id, seller) VALUES ('$productName', '$price', '$description', '$seller', '".$_SESSION['name']."')";
$conn->query($sql);
if($conn->error){
...
$sql = "SELECT id FROM produkty WHERE prod_name = '$productName' AND price = '$price' AND description = '$description' AND user_id = '$seller' AND seller = '".$_SESSION['name']."'";
$conn->query($sql);
if($conn->error){
    echo "Error: ".$conn->error;
    exit;
...
$sql = "UPDATE produkty SET prod_thumbnail = '$new_file_name' WHERE id = '$id'";
$conn->query($sql);
if($conn->error){
    echo "Error: ".$conn->error;
    exit;
