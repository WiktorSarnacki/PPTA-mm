    ...
    if(isset($_SESSION['zalogowany'])){
        if($_SESSION['zalogowany'] == true){
            ...
            if(isset($_POST['buy'])){
                ...
                echo "<button type='submit' name='finalize'>Kup</button>";
                echo "</form>";
                echo "<form action='cart.php' method='post'>";
                echo "<button type='submit' name='cancel'>Anuluj</button>";
                echo "</form>";
                echo "</div>";
            }
            ...
            $sql = "SELECT produkty.id ,produkty.prod_name, produkty.price, produkty.seller_id, produkty.prod_thumbnail, cart.ilosc FROM produkty, cart WHERE cart.id_uzytkownika = ".$_SESSION['userID']." and cart.id_produktu = produkty.id;"; ##zmienione
            $result = $conn->query($sql);
            if($result->num_rows > 0){
                while($row = mysqli_fetch_assoc($result)){
                    $name = $conn->query("SELECT name FROM userdata WHERE userdata.id = ".$row['seller_id'])->fetch_assoc(); ##dodane
                    $strName = $name['name']; ##dodane
                    $ilosc = $row['ilosc'];
                    echo "<div id='produkt'>";
                    ...
                    echo "<p>Sprzedwaca:".$strName."</p>"; ##zmienione
                    echo "</div>";
                    ...
                    echo "Zmień ilość:<input type='range' value=".$ilosc." min=1 max=99 name='change' oninput='this.nextElementSibling.value = this.value'><output>".$ilosc."</output>";
                    echo "<button type='submit' name='changeCart'>Zapisz</button>";
                    echo "</form>";
                    echo "</div>";
                    echo "<div id='usun'>";
                    echo "<form action='cart.php' method='post'>";
                    echo "<input type='hidden' name='id-produktu' value='".$row['id']."'>";
                    echo "<button type='submit' name='removecart'>Usuń z koszyka</button>";
                    echo "</form>";
                    echo "</div>";
                    echo "<form action='cart.php' method='post'>";
                    echo "<input type='hidden' name='id-produktu' value='".$row['id']."'>";
                    echo "<button type='submit' name='buy'>Kup</button>";
                    echo "</form>";
                    echo "</div>";
                    echo "</div>";
            }
